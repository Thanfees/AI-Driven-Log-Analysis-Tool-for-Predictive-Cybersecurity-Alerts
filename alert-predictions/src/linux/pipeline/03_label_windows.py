#!/usr/bin/env python3
"""
03_label_windows.py - Apply rule-based anomaly labels to windowed log data.

Uses pattern matching to identify anomalous windows based on security and
system events.

Usage:
    python src/linux/pipeline/03_label_windows.py \\
        --input data/linux/processed/logs_windowz.csv \\
        --output data/linux/labeled/logs_windowz_labeled.csv \\
...
try:
    ...
    LABELING_RULES = [
        ...
        ("AUTH_FAIL", re.compile(r"\bFailed password\b|\binvalid user\b|\bauthentication failure\b", re.IGNORECASE)),
...
def main() -> None:
    """Main entry point for window labeling script."""
    setup_logging()
    
    ap = argparse.ArgumentParser(
        description="Apply rule-based anomaly labels to windowed log data."
    )
    ...
    logger.info(f"Applying {len(LABELING_RULES)} labeling rules...")
