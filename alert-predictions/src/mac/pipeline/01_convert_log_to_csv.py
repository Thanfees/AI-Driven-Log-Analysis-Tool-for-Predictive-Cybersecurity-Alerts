#!/usr/bin/env python3
"""
01_convert_log_to_csv.py - Parse macOS syslog-formatted .log files and export CSV.

Handles macOS syslog format with extended process naming (e.g., com.apple.xpc.launchd).

Usage:
    # Single file
    python src/mac/pipeline/01_convert_log_to_csv.py \\
        --log-path raw_logs/mac/Mac.log \\
        --output data/mac/raw_csv/Mac.log.csv

    # Directory
    python src/mac/pipeline/01_convert_log_to_csv.py \\
        --log-path raw_logs/mac \\
        --output-dir data/mac/raw_csv \\
        --recursive
"""
...
def parse_log(
    ...
    """
    Parse a macOS syslog file and extract structured fields.
...
def main() -> None:
    """Main entry point for Mac log conversion script."""
    setup_logging()

    parser = argparse.ArgumentParser(
        description="Convert macOS syslog text to CSV (file or folder)."
    )
    parser.add_argument(
        "--log-path",
        default="raw_logs/mac",
